 ...
 */
export function fromCredentialAndDid(credential, attesterDid) {
    const attestation = {
 ...
 */
export async function getDelegationDetails(input) {
    if (input === null) {
 ...
 */
export function isIAttestation(input) {
    try {
 ...
 */
export function verifyAgainstCredential(attestation, credential) {
    if (credential.claim.cTypeHash !== attestation.cTypeHash ||
        credential.rootHash !== attestation.claimHash) {
        throw new SDKErrors.CredentialUnverifiableError('Attestation does not match credential');
    }
