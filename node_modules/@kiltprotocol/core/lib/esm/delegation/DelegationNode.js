...
import { addDelegationToChainArgs, getAttestationHashes, getChildren, fetch, } from './DelegationNode.chain.js';
...
export class DelegationNode {
    ...
    async delegateSign(delegateDid, sign) {
        ...
        if (!fragment) {
            throw new SDKErrors.DidError(`DID key uri "${delegateSignature.keyUri}" couldn't be parsed`);
        }
        const key = Did.getKey(delegateDid, fragment);
        if (!key) {
            throw new SDKErrors.DidError(`Key with fragment "${fragment}" was not found on DID: "${delegateDid.uri}"`);
        }
    ...
    async getStoreTx(signature) {
        ...
        return api.tx.delegation.addDelegation(...addDelegationToChainArgs(this, signature));
    }
    isRoot() {
    ...
    async getRevokeTx(did) {
        ...
        if (!node) {
            throw new SDKErrors.UnauthorizedError(`The DID "${did}" is not among the delegators and may not revoke this node`);
        }
