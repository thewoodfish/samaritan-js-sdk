...
exports.didSignatureToChain = exports.generateDidAuthenticatedTx = exports.getStoreTx = exports.publicKeyToChain = exports.serviceFromChain = exports.serviceToChain = exports.validateService = exports.documentFromChain = exports.fromChain = exports.depositFromChain = exports.resourceIdToChain = exports.toChain = void 0;
 ...
 */
function toChain(did) {
    return (0, Did_utils_js_1.parse)(did).address;
}
...
exports.resourceIdToChain = resourceIdToChain;
/**
...
function depositFromChain(deposit) {
    return {
        owner: utils_1.Crypto.encodeAddress(deposit.owner, utils_1.ss58Format),
        amount: deposit.amount.toBn(),
    };
}
exports.depositFromChain = depositFromChain;
...
function fromChain(encoded) {
    return (0, Did_utils_js_1.getFullDidUri)(utils_1.Crypto.encodeAddress(encoded, utils_1.ss58Format));
}
...
function documentFromChain(encoded) {
    const { publicKeys, authenticationKey, attestationKey, delegationKey, keyAgreementKeys, lastTxCounter, deposit, } = encoded.unwrap();
...
exports.documentFromChain = documentFromChain;
/**
 ...
 */
function validateService(endpoint) {
    const { id, serviceEndpoint } = endpoint;
    if (id.startsWith('did:kilt')) {
        throw new utils_1.SDKErrors.DidError(`This function requires only the URI fragment part (following '#') of the service ID, not the full DID URI, which is violated by id "${id}"`);
    }
    if (!isUriFragment(resourceIdToChain(id))) {
        throw new utils_1.SDKErrors.DidError(`The service ID must be valid as a URI fragment according to RFC#3986, which "${id}" is not. Make sure not to use disallowed characters (e.g. whitespace) or consider URL-encoding the desired id.`);
    }
    serviceEndpoint.forEach((uri) => {
        if (!isUri(uri)) {
            throw new utils_1.SDKErrors.DidError(`A service URI must be a URI according to RFC#3986, which "${uri}" (service id "${id}") is not. Make sure not to use disallowed characters (e.g. whitespace) or consider URL-encoding resource locators beforehand.`);
        }
    });
}
 ...
 */
function serviceToChain(service) {
...
exports.serviceToChain = serviceToChain;
/**
...
exports.serviceFromChain = serviceFromChain;
/**
 ...
 */
function publicKeyToChain(key) {
    // TypeScript can't infer type here, so we have to add a type assertion.
    return { [key.type]: key.publicKey };
}
exports.publicKeyToChain = publicKeyToChain;
/**
 ...
 */
async function getStoreTx(input, submitter, sign) {
    const api = config_1.ConfigService.get('api');
    const { authentication, assertionMethod, capabilityDelegation, keyAgreement = [], service = [], } = input;
    if (!('authentication' in input) || typeof authentication[0] !== 'object') {
        throw new utils_1.SDKErrors.DidError(`The provided DID does not have an authentication key to sign the creation operation`);
    }
    // For now, it only takes the first attestation key, if present.
    if (assertionMethod && assertionMethod.length > 1) {
        throw new utils_1.SDKErrors.DidError(`More than one attestation key (${assertionMethod.length}) specified. The chain can only store one.`);
    }
    // For now, it only takes the first delegation key, if present.
    if (capabilityDelegation && capabilityDelegation.length > 1) {
        throw new utils_1.SDKErrors.DidError(`More than one delegation key (${capabilityDelegation.length}) specified. The chain can only store one.`);
    }
    const maxKeyAgreementKeys = api.consts.did.maxNewKeyAgreementKeys.toNumber();
    if (keyAgreement.length > maxKeyAgreementKeys) {
        throw new utils_1.SDKErrors.DidError(`The number of key agreement keys in the creation operation is greater than the maximum allowed, which is ${maxKeyAgreementKeys}`);
    }
    const maxNumberOfServicesPerDid = api.consts.did.maxNumberOfServicesPerDid.toNumber();
    if (service.length > maxNumberOfServicesPerDid) {
        throw new utils_1.SDKErrors.DidError(`Cannot store more than ${maxNumberOfServicesPerDid} service endpoints per DID`);
    }
    const [authenticationKey] = authentication;
    const did = (0, Did_utils_js_1.getAddressByKey)(authenticationKey);
    const newAttestationKey = assertionMethod &&
        assertionMethod.length > 0 &&
        publicKeyToChain(assertionMethod[0]);
    const newDelegationKey = capabilityDelegation &&
 ...
 */
async function generateDidAuthenticatedTx({ did, keyRelationship, sign, call, txCounter, submitter, blockNumber, }) {
    const api = config_1.ConfigService.get('api');
    const signableCall = api.registry.createType(api.tx.did.submitDidCall.meta.args[0].type.toString(), {
        txCounter,
        did: toChain(did),
        call,
        submitter,
 ...
 * @returns Data restructured to allow SCALE encoding by polkadot api.
 */
function didSignatureToChain(key, signature) {
    if (!types_1.verificationKeyTypes.includes(key.type)) {
        throw new utils_1.SDKErrors.DidError(`encodedDidSignature requires a verification key. A key of type "${key.type}" was used instead`);
    }
    return { [key.type]: signature };
}
exports.didSignatureToChain = didSignatureToChain;
