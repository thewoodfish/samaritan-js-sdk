...
function verifyDidSignatureDataStructure(input) {
    ...
    if (!isHex(input.signature)) {
        throw new SDKErrors.SignatureMalformedError(`Expected signature as a hex string, got ${input.signature}`);
    }
 ...
 */
export async function verifyDidSignature({ message, signature, keyUri, expectedSigner, allowUpgraded = false, expectedVerificationMethod, didResolveKey = resolveKey, }) {
 ...
 */
export function isDidSignature(input) {
    try {
 ...
 */
export function signatureToJson({ signature, keyUri, }) {
 ...
 */
export function signatureFromJson(input) {
    const keyUri = 'keyUri' in input ? input.keyUri : input.keyId;
