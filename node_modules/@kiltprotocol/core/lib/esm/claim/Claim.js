 ...
 */
export function toJsonLD(claim, expanded = true) {
 ...
 */
export function hashClaimContents(claim, options = {}) {
    // apply defaults
    ...
    processed.forEach(({ digest, nonce, statement }) => {
        ...
        if (!nonce)
            throw new SDKErrors.ClaimNonceMapMalformedError(statement);
 ...
 * @param options.hasher The hasher to be used. Required but defaults to 256 bit blake2 over `${nonce}${statement}`.
 */
export function verifyDisclosedAttributes(claim, proof, options = {}) {
    // apply defaults
    const defaults = { canonicalisation: makeStatementsJsonLD };
    const canonicalisation = options.canonicalisation || defaults.canonicalisation;
    const { nonces } = proof;
    ...
    const { verified, errors } = hashed.reduce((status, { saltedHash, statement, digest, nonce }) => {
        ...
        if (!proof.hashes.includes(saltedHash)) {
            status.errors.push(new SDKErrors.InvalidProofForStatementError(statement));
            return { ...status, verified: false };
        }
        return status;
    }, { verified: true, errors: [] });
    if (verified !== true) {
        throw new SDKErrors.ClaimUnverifiableError('One or more statements in the claim could not be verified', { cause: errors });
    }
}
/**
 ...
 */
export function verify(claimInput, cType) {
 ...
 */
export function fromNestedCTypeClaim(cTypeInput, nestedCType, claimContents, claimOwner) {
 ...
 */
export function fromCTypeAndClaimContents(cType, claimContents, claimOwner) {
 ...
 */
export function isIClaim(input) {
    try {
