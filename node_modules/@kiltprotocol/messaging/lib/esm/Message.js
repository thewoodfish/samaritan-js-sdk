...
import { hexToU8a, stringToU8a, u8aToHex, u8aToString, isHex, isJsonObject, } from '@polkadot/util';
/**
 * Checks if delegation data is well formed.
 *
 * @param delegationData Delegation data to check.
 */
export function verifyDelegationStructure(delegationData) {
    ...
    if (permissions.length === 0 || permissions.length > 3) {
        throw new SDKErrors.UnauthorizedError('Must have at least one permission and no more then two');
    }
 ...
 */
export function verifyMessageBody(body) {
    switch (body.type) {
        case 'request-terms': {
        ...
        case 'reject-terms': {
            ...
            body.content.legitimations.forEach((val) => Credential.verifyDataStructure(val));
            break;
        }
 ...
 */
export function verifyMessageEnvelope(message) {
 ...
 */
export function verifyRequiredCTypeProperties(requiredProperties, cType) {
 ...
 */
export function ensureOwnerIsSender({ body, sender }) {
    switch (body.type) {
        case 'request-attestation':
            {
 ...
 */
export async function decrypt(encrypted, decryptCallback, { resolveKey = Did.resolveKey, } = {}) {
 ...
 */
export function verify(decryptedMessage) {
 ...
 */
export function fromBody(body, sender, receiver) {
    return {
        body,
        createdAt: Date.now(),
        receiver,
 ...
 */
export async function encrypt(message, encryptCallback, receiverKeyUri, { resolveKey = Did.resolveKey, } = {}) {
