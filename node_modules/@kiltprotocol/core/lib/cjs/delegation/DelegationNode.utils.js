...
exports.errorCheck = exports.countNodeDepth = exports.permissionsAsBitset = void 0;
...
function permissionsAsBitset(delegation) {
    const permissionsBitsetData = delegation.permissions.reduce((accumulator, currentValue) => accumulator + currentValue);
 ...
 */
async function countNodeDepth(attester, attestation) {
    ...
    if (attestation.owner !== attester && attestation.delegationId !== null) {
        ...
        try {
            const delegationNode = await DelegationNode_js_1.DelegationNode.fetch(attestation.delegationId);
            const { steps, node } = await delegationNode.findAncestorOwnedBy(attester);
            delegationTreeTraversalSteps += steps;
            if (node === null) {
                throw new utils_1.SDKErrors.UnauthorizedError('Attester is not authorized to revoke this attestation. (Attester not in delegation tree)');
            }
        }
        catch {
            // ignore the error
        ...
        throw new utils_1.SDKErrors.UnauthorizedError('Attester is not authorized to revoke this attestation. (Not the owner, no delegations)');
    }
    return delegationTreeTraversalSteps;
}
exports.countNodeDepth = countNodeDepth;
/**
 * Checks for errors on delegation node data.
 *
 * @param delegationNodeInput Delegation node data.
 */
function errorCheck(delegationNodeInput) {
    ...
    else if (typeof rootId !== 'string' || !(0, util_1.isHex)(rootId)) {
