...
exports.associateAccountToChainArgs = exports.accountToChain = void 0;
...
function isEthereumEnabled(api) {
    const removeType = api.createType(api.tx.didLookup.removeAccountAssociation.meta.args[0]?.type?.toString() ||
        'bool');
    const associateType = api.createType(api.tx.didLookup.associateAccount.meta.args[0]?.type?.toString() || 'bool');
...
async function associateAccountToChainArgs(accountAddress, did, sign, nBlocksValid = 10) {
    ...
    const encoded = api
        .createType(`(${DidAddress}, ${BlockNumber})`, [(0, Did_chain_js_1.toChain)(did), validTill])
        .toU8a();
    const isAccountId32 = (0, util_crypto_1.decodeAddress)(accountAddress).length > 20;
    const length = (0, util_1.stringToU8a)(String(encoded.length));
    const paddedDetails = (0, util_1.u8aToHex)(isAccountId32
        ? (0, util_1.u8aWrapBytes)(encoded)
        : (0, util_1.u8aConcatStrict)([util_1.U8A_WRAP_ETHEREUM, length, encoded]));
    const { signature, type } = getUnprefixedSignature(paddedDetails, await sign(paddedDetails), accountAddress);
