 ...
 */
export function serializeForHash(cType) {
 ...
 */
export function getHashForSchema(cType) {
...
export function idToHash(id) {
    ...
    if (!result)
        throw new SDKErrors.CTypeHashMissingError(`The string ${id} is not a valid CType id`);
    return result[1];
}
/**
 ...
 * @returns Schema id uri.
 */
export function getIdForSchema(schema) {
 ...
 */
export function verifyObjectAgainstSchema(object, schema, messages) {
 ...
 */
export function verifyClaimAgainstSchema(claimContents, schema, messages) {
 ...
 */
export async function verifyStored(ctype) {
    const api = ConfigService.get('api');
    const hash = idToHash(ctype.$id);
    const encoded = await api.query.ctype.ctypes(hash);
    if (encoded.isNone)
        throw new SDKErrors.CTypeHashMissingError(`CType with hash ${hash} is not registered on chain`);
}
/**
 ...
 * @param messages - Optional empty array. If passed, this receives all verification errors.
 */
export function verifyClaimAgainstNestedSchemas(cType, nestedCTypes, claimContents, messages) {
 ...
 */
export function verifyCTypeMetadata(metadata) {
 ...
 */
export function fromProperties(title, properties) {
    const schema = {
        properties,
        title,
 ...
 */
export function isICType(input) {
    try {
