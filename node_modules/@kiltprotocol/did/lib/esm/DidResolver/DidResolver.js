 ...
 */
export function keyToResolvedKey(key, did) {
 ...
 */
export function resolvedKeyToKey(key) {
    const { id, publicKey, includedAt, type } = key;
    return {
 ...
 */
export async function resolveKey(keyUri, expectedVerificationMethod) {
    const { did, fragment: keyId } = parse(keyUri);
    // A fragment (keyId) IS expected to resolve a key.
    if (!keyId) {
        throw new SDKErrors.DidError(`Key URI "${keyUri}" is not a valid DID resource`);
    }
    ...
    if (expectedVerificationMethod &&
        ...
        throw new SDKErrors.DidError(`No key "${keyUri}" for the verification method "${expectedVerificationMethod}"`);
    }
 ...
 */
export async function resolveService(serviceUri) {
    const { did, fragment: serviceId } = parse(serviceUri);
    // A fragment (serviceId) IS expected to resolve a key.
    if (!serviceId) {
        throw new SDKErrors.DidError(`Service URI "${serviceUri}" is not a valid DID resource`);
    }
    const resolved = await resolve(did);
    if (!resolved) {
        throw new SDKErrors.DidNotFoundError();
