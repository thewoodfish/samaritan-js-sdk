...
exports.parseDocumentFromLightDid = exports.createLightDidDocument = void 0;
...
function validateCreateDocumentInput({ authentication, keyAgreement, service: services, }) {
    ...
    if (!authenticationKeyTypeEncoding) {
        throw new utils_1.SDKErrors.UnsupportedKeyError(authentication[0].type);
    }
    if (keyAgreement?.[0].type &&
        !types_1.encryptionKeyTypes.includes(keyAgreement[0].type)) {
        throw new utils_1.SDKErrors.DidError(`Encryption key type "${keyAgreement[0].type}" is not supported`);
    }
    ...
    services?.forEach((service) => {
        ...
        if (service.id === '#authentication' || service.id === '#encryption') {
            throw new utils_1.SDKErrors.DidError(`Cannot specify a service ID with the name "${service.id}" as it is a reserved keyword`);
        }
        (0, Did_chain_js_1.validateService)(service);
    });
}
...
function serializeAdditionalLightDidDetails({ keyAgreement, service, }) {
    ...
    if (service && service.length > 0) {
        objectToSerialize[SERVICES_MAP_KEY] = service.map(({ id, ...rest }) => ({
            id: (0, Did_chain_js_1.resourceIdToChain)(id),
            ...rest,
        }));
    }
    ...
    const serialized = (0, cbor_1.encode)(objectToSerialize);
    return (0, util_crypto_1.base58Encode)([serializationVersion, ...serialized], true);
}
function deserializeAdditionalLightDidDetails(rawInput, version = 1) {
    if (version !== 1) {
        throw new utils_1.SDKErrors.DidError('Serialization version not supported');
    }
    const decoded = (0, util_crypto_1.base58Decode)(rawInput, true);
    const serializationVersion = decoded[0];
    const serialized = decoded.slice(1);
    if (serializationVersion !== 0x0) {
        throw new utils_1.SDKErrors.DidError('Serialization algorithm not supported');
    }
    const deserialized = (0, cbor_1.decode)(serialized);
 ...
 */
function createLightDidDocument({ authentication, keyAgreement = undefined, service, }) {
    ...
    const authenticationKeyTypeEncoding = verificationKeyTypeToLightDidEncoding[authentication[0].type];
    const address = (0, Did_utils_js_1.getAddressByKey)(authentication[0]);
...
exports.createLightDidDocument = createLightDidDocument;
/**
 ...
 * @returns The resulting [[DidDocument]].
 */
function parseDocumentFromLightDid(uri, failIfFragmentPresent = true) {
    const { address, version, encodedDetails, fragment, type, authKeyTypeEncoding, } = (0, Did_utils_js_1.parse)(uri);
    if (type !== 'light') {
        throw new utils_1.SDKErrors.DidError(`Cannot build a light DID from the provided URI "${uri}" because it does not refer to a light DID`);
    }
    if (fragment && failIfFragmentPresent) {
        throw new utils_1.SDKErrors.DidError(`Cannot build a light DID from the provided URI "${uri}" because it has a fragment`);
    }
    const keyType = authKeyTypeEncoding &&
        lightDidEncodingToVerificationKeyType[authKeyTypeEncoding];
    if (keyType === undefined) {
        throw new utils_1.SDKErrors.DidError(`Authentication key encoding "${authKeyTypeEncoding}" does not match any supported key type`);
    }
    const publicKey = (0, util_crypto_1.decodeAddress)(address, false, utils_1.ss58Format);
...
exports.parseDocumentFromLightDid = parseDocumentFromLightDid;
