...
exports.isIClaim = exports.fromCTypeAndClaimContents = exports.fromNestedCTypeClaim = exports.verify = exports.verifyDataStructure = exports.verifyDisclosedAttributes = exports.hashClaimContents = exports.toJsonLD = void 0;
...
const CType = tslib_1.__importStar(require("../ctype/index.js"));
 ...
 */
function jsonLDcontents(claim, expanded = true) {
 ...
 */
function toJsonLD(claim, expanded = true) {
...
function makeStatementsJsonLD(claim) {
 ...
 */
function hashClaimContents(claim, options = {}) {
    // apply defaults
    ...
    processed.forEach(({ digest, nonce, statement }) => {
        ...
        if (!nonce)
            throw new utils_1.SDKErrors.ClaimNonceMapMalformedError(statement);
        nonceMap[digest] = nonce;
    }, {});
    return { hashes, nonceMap };
}
exports.hashClaimContents = hashClaimContents;
/**
 ...
 * @param options.hasher The hasher to be used. Required but defaults to 256 bit blake2 over `${nonce}${statement}`.
 */
function verifyDisclosedAttributes(claim, proof, options = {}) {
    // apply defaults
    const defaults = { canonicalisation: makeStatementsJsonLD };
    const canonicalisation = options.canonicalisation || defaults.canonicalisation;
    const { nonces } = proof;
    ...
    if (verified !== true) {
        throw new utils_1.SDKErrors.ClaimUnverifiableError('One or more statements in the claim could not be verified', { cause: errors });
    }
}
 ...
 */
function verify(claimInput, cType) {
 ...
 */
function fromNestedCTypeClaim(cTypeInput, nestedCType, claimContents, claimOwner) {
...
exports.fromNestedCTypeClaim = fromNestedCTypeClaim;
/**
 ...
 */
function fromCTypeAndClaimContents(cType, claimContents, claimOwner) {
...
exports.fromCTypeAndClaimContents = fromCTypeAndClaimContents;
/**
