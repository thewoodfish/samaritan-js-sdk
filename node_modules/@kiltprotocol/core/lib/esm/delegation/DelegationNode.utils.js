 ...
 */
export function permissionsAsBitset(delegation) {
 ...
 */
export async function countNodeDepth(attester, attestation) {
    ...
    if (attestation.owner !== attester && attestation.delegationId !== null) {
        ...
        try {
            const delegationNode = await DelegationNode.fetch(attestation.delegationId);
            const { steps, node } = await delegationNode.findAncestorOwnedBy(attester);
            delegationTreeTraversalSteps += steps;
            if (node === null) {
                throw new SDKErrors.UnauthorizedError('Attester is not authorized to revoke this attestation. (Attester not in delegation tree)');
            }
        }
        catch {
            // ignore the error
        ...
        throw new SDKErrors.UnauthorizedError('Attester is not authorized to revoke this attestation. (Not the owner, no delegations)');
    }
 ...
 */
export function errorCheck(delegationNodeInput) {
