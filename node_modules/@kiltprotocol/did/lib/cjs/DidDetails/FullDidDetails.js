...
exports.authorizeBatch = exports.authorizeTx = exports.getKeyRelationshipForTx = void 0;
...
function getKeyRelationshipForMethod(call) {
 ...
 */
function getKeyRelationshipForTx(extrinsic) {
    return getKeyRelationshipForMethod(extrinsic.method);
}
exports.getKeyRelationshipForTx = getKeyRelationshipForTx;
// Max nonce value is (2^64) - 1
const maxNonceValue = new util_1.BN(2).pow(new util_1.BN(64)).subn(1);
function increaseNonce(currentNonce, increment = 1) {
 ...
 */
async function getNextNonce(did) {
    const api = config_1.ConfigService.get('api');
    const queried = await api.query.did.did((0, Did_chain_js_1.toChain)(did));
    const currentNonce = queried.isSome
        ? (0, Did_chain_js_1.documentFromChain)(queried).lastTxCounter
        : new util_1.BN(0);
 ...
 */
async function authorizeTx(did, extrinsic, sign, submitterAccount, { txCounter, } = {}) {
    if ((0, Did_utils_js_1.parse)(did).type === 'light') {
        throw new utils_1.SDKErrors.DidError(`An extrinsic can only be authorized with a full DID, not with "${did}"`);
    }
    const keyRelationship = getKeyRelationshipForTx(extrinsic);
    if (keyRelationship === undefined) {
        throw new utils_1.SDKErrors.SDKError('No key relationship found for extrinsic');
    }
    return (0, Did_chain_js_1.generateDidAuthenticatedTx)({
        did,
        keyRelationship,
        sign,
        call: extrinsic,
...
exports.authorizeTx = authorizeTx;
function groupExtrinsicsByKeyRelationship(extrinsics) {
    const [first, ...rest] = extrinsics.map((extrinsic) => {
        const keyRelationship = getKeyRelationshipForTx(extrinsic);
        if (!keyRelationship) {
            throw new utils_1.SDKErrors.DidBatchError('Can only batch extrinsics that require a DID signature');
        }
...
async function authorizeBatch({ batchFunction, did, extrinsics, nonce, sign, submitter, }) {
    if (extrinsics.length === 0) {
        throw new utils_1.SDKErrors.DidBatchError('Cannot build a batch with no transactions');
    }
    if ((0, Did_utils_js_1.parse)(did).type === 'light') {
        throw new utils_1.SDKErrors.DidError(`An extrinsic can only be authorized with a full DID, not with "${did}"`);
    }
...
exports.authorizeBatch = authorizeBatch;
