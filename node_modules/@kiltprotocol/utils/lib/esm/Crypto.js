...
import { isString, stringToU8a, u8aConcat, u8aToHex, u8aToString, u8aToU8a, } from '@polkadot/util';
import { blake2AsHex, blake2AsU8a, naclBoxPairFromSecret, randomAsU8a, signatureVerify, } from '@polkadot/util-crypto';
 ...
 */
export function coToUInt8(input, hexAsString = false) {
 ...
 */
export function signStr(message, signKeyPair) {
 ...
 */
export function verify(message, signature, addressOrPublicKey) {
 ...
 */
export function hash(value, bitLength) {
 ...
 */
export function encodeObjectAsStr(value) {
    const input = 
 ...
 */
export function encryptAsymmetric(message, publicKeyA, secretKeyB) {
    const nonce = nacl.randomBytes(24);
    const box = nacl.box(coToUInt8(message, true), nonce, coToUInt8(publicKeyA), coToUInt8(secretKeyB));
    return { box, nonce };
}
/**
 * Wrapper around nacl.box. Authenticated encryption of a message for a recipient's public key.
 ...
 */
export function encryptAsymmetricAsStr(message, publicKeyA, secretKeyB) {
 ...
 */
export function decryptAsymmetric(data, publicKeyB, secretKeyA) {
    const decrypted = nacl.box.open(coToUInt8(data.box), coToUInt8(data.nonce), coToUInt8(publicKeyB), coToUInt8(secretKeyA));
 ...
 */
export function decryptAsymmetricAsStr(data, publicKeyB, secretKeyA) {
 ...
 */
export function saltedBlake2b256(value, nonce = '') {
 ...
 */
export function hashStatements(statements, options = {}) {
    // apply defaults
    const defaults = {
 ...
 * @returns The keypair.
 */
export function makeKeypairFromSeed(seed = randomAsU8a(32), type) {
 ...
 * @returns The keypair.
 */
export function makeKeypairFromUri(uri, type) {
    const keyring = new Keyring({ ss58Format, type });
 ...
 * @returns The keypair.
 */
export function makeEncryptionKeypairFromSeed(seed = randomAsU8a(32)) {
    return {
