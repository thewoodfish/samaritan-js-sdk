...
import { documentFromChain, generateDidAuthenticatedTx, toChain, } from '../Did.chain.js';
 ...
 */
export function getKeyRelationshipForTx(extrinsic) {
 ...
 */
export async function authorizeTx(did, extrinsic, sign, submitterAccount, { txCounter, } = {}) {
    if (parse(did).type === 'light') {
        throw new SDKErrors.DidError(`An extrinsic can only be authorized with a full DID, not with "${did}"`);
    }
...
function groupExtrinsicsByKeyRelationship(extrinsics) {
    const [first, ...rest] = extrinsics.map((extrinsic) => {
        const keyRelationship = getKeyRelationshipForTx(extrinsic);
        if (!keyRelationship) {
            throw new SDKErrors.DidBatchError('Can only batch extrinsics that require a DID signature');
        }
 ...
 * @returns The DID-signed submittable extrinsic.
 */
export async function authorizeBatch({ batchFunction, did, extrinsics, nonce, sign, submitter, }) {
    if (extrinsics.length === 0) {
        throw new SDKErrors.DidBatchError('Cannot build a batch with no transactions');
    }
    if (parse(did).type === 'light') {
        throw new SDKErrors.DidError(`An extrinsic can only be authorized with a full DID, not with "${did}"`);
    }
